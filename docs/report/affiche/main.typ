...
#figure(
  image("../images/Schema-Resume.png", width: 90%),
  caption: [Architecture globale de l'infrastructure.],
)

= Résultats
Le run de production traite les 14'207 images de la ville de Vevey en 10h23 sur trois GPUs L40S et produit 423'819 détections. Le passage d'un à trois GPUs accélère le traitement de 2,99×, un gain quasi linéaire. L'inférence par tuiles se parallélise sans goulot d'étranglement et le débit croît avec le nombre de GPUs disponibles.
...
#figure(
  image("../images/firstOutputLabelStudio.png", width: 100%),
  caption: [Pré-annotations SAM3 importées dans Label Studio.],
)

= Conclusion
La pipeline transforme une tâche de plusieurs mois pour un homme, en un traitement d'une nuit sur l'infrastructure existante, et le seul maillon automatisable de la chaîne de production en est devenu le poste le moins cher. L'architecture reste modulaire, le modèle est confiné dans une seule classe et peut être remplacé par un successeur en quelques dizaines de lignes, le tuilage, la distribution et la reprise n'y touchent pas. Les perspectives portent sur la calibration du recouvrement à partir de la taille réelle des objets et sur la montée en charge du nombre de GPUs.
