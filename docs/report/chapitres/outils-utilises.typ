...
#figure(
  table(
    ...
    [pytest], [—], [Suite de tests unitaires (`tests/unit`), Ray/PyTorch mockés],
    [GitHub Actions], [—], [CI : exécute la suite pytest avant chaque build d'image],
    [Claude (Anthropic)],
    [—],
    [Assistant IA employé comme correcteur du rapport (relecture orthographique et grammaticale, reformulation de phrases, cohérence entre chapitres) ainsi que comme aide au débogage et au développement de l'interface web de la console. Les choix techniques et la rédaction restent l'œuvre de l'auteur, qui a relu et validé chaque contribution],
  ),
