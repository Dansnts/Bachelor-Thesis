...
Si les GPUs voulus par l'affinité sont occupés, le scheduler renvoie `Insufficient nvidia.com/gpu`.

Le throughput#footnote[Taux d'images ou de tuiles traitées par seconde.] croît linéairement avec le nombre de workers tant que les tuiles d'une image sont réparties sur des GPUs distincts, l'étape d'inférence est *embarrassingly parallel*, c'est-à-dire que chaque tuile est traitée de façon indépendante.
...
Le stride de 768 est donc un compromis raisonné, non un optimum, sa valeur idéale se calibrerait sur la distribution réelle des tailles d'objets, ce qui reste un axe d'amélioration mais sortirait du cadre de ce travail étant donné le nombre de tests supplémentaires.
...
=== La résolution d'entrée figée de SAM3
...
La contrainte d'entrée décrite plus haut rend la taille de tuile libre (le mécanisme de redimensionnement est détaillé au chapitre implémentation, cf. @implementation). Nous croisons la taille native 1008×1008 (sans redimensionnement) et 504×504 (la moitié exacte, agrandie ×2 avant le modèle) avec les quatre facteurs de downsampling. Une tuile plus grande serait simplement réduite à 1008, soit l'équivalent d'un downsampling, sans intérêt ici.

Le temps mesuré couvre le traitement d'une image (tuilage, inférence, polygonisation, écriture du résultat), hors chargement du modèle (≈ 54 s, amorti sur un batch). Le score est la confiance moyenne de SAM3 (0 à 1) sur les polygones retenus. Tous les runs de cette section ont été exécutés sur une L40S (`iict-suchet`).
...
Après discussion avec mon collègue Valentin, qui travaillait sur la partie analyse de NearAI, nous avons remarqué que les bouches d'égout et plaques de canalisation sont parfois mal, voire très mal, détectées. La cause est simple : un béton sombre a souvent une couleur très proche de celle d'une plaque de canalisation.
...
#figure(
  image("../images/sam3BadView2.png", width: 100%),
...
#figure(
  image("../images/sam3BadView3.png", width: 100%),
...
En résumé, pour éviter les faux positifs et obtenir les meilleurs résultats, il faut utiliser un *vocabulaire spécifique au domaine routier*, garder une *tuile standard de 1008* et exploiter un *downsampling de 0,75*.
...
Le temps total ne double pas avec le nombre de labels. Passer de 3 à 6 labels (×2) n'allonge le run que de 8 h 04 à 10 h 23 (×1,29), et le nombre de détections ne croît que de 397'741 à 423'819. L'essentiel du temps est un coût fixe (téléchargement, downsampling, tuilage, I/O), indépendant du nombre de labels. Seule la partie inférence croît avec eux, à raison d'une requête `FindQuery` par label et par tuile. Entre ces deux runs, chaque label supplémentaire ajoute ≈ 46 min (2'777 s) au-dessus d'un coût fixe estimé à ≈ 5 h 45.
...
=== Comparaison avec l'annotation humaine
...
Au gain de temps s'ajoute la fatigue mentale : annoter en série est une tâche répétitive et usante pour un humain, alors qu'elle ne coûte à la machine que quelques secondes de calcul par image. Le confort de l'annotateur, difficile à chiffrer, compte donc autant que les minutes gagnées.
...
#figure(
  ...
  caption: [Dashboard Grafana des runs batch pendant le run de production Vevey (47 % de progression) : tuiles d'état, temps d'inférence par image, progression et détections extraites des logs Loki.],
) <fig-batch-running>
...
#figure(
  ...
  ```,
  caption: [Dernières lignes de log d'un run batch. Elles alimentent les compteurs du dashboard Grafana pour offrir un résultat lisible sans fouiller les logs.],
) <fig-batch-final-logs>


=== Logs Ray dans Grafana

Alloy, successeur de Promtail, collecte les `stdout`/`stderr` de chaque pod du namespace et les expédie vers Loki, stocké sur MinIO (bucket dédié `nearai-logs`, rétention 30 jours). Les logs se requêtent ensuite en LogQL par label, ce qui permet de filtrer une catégorie précise, ici la progression du run de production :
...
#figure(
  ...
  caption: [Vue k9s des pods du namespace après une journée de tests : la stack d'observabilité (Alloy, Grafana, Loki, Prometheus), le head Ray et ses trois workers GPU, l'API, puis les pods drivers des runs batch et solo terminés (`Completed`).],
) <fig-pods-k9s>

La colonne `NODE` matérialise la politique de placement : seuls les pods à GPU sont épinglés, les trois workers Ray tournent sur `iict-suchet` (L40S), tandis que les pods sans GPU (head Ray, API, drivers batch) se répartissent librement sur `iict-chasseron` et `iict-k8s-node4-rad` au gré du scheduler. Les GPUs ne portent ainsi que de l'inférence, jamais de l'orchestration.

La colonne `RESTARTS` est à zéro sur toute la colonne, y compris pour l'API en ligne depuis trois jours et pour Alloy et Prometheus, en place depuis 48 jours. Les consommations confirment le dimensionnement du chapitre architecture : le head occupe 2,1 Gio, soit la moitié de ses 4 Gio, les workers au repos retombent à ≈ 0,5 Gio et quelques pourcents de CPU (le gros de l'inférence vit en VRAM), et la stack d'observabilité complète tient dans ≈ 460 Mio cumulés. L'API illustre au passage l'intérêt de la marge entre requête et limite : elle dépasse sa requête mémoire (181 %) tout en restant à 45 % de sa limite, sans jamais être tuée.
...
En pratique, au-delà d'une à deux connexions concurrentes le démarrage du serveur suivant échoue (`Starting Ray client server failed`) et le driver s'arrête sur `ConnectionAbortedError`. Le driver retente sa connexion plusieurs fois avant d'abandonner (cf. @problemes-driver), puis le `backoffLimit` du Job Kubernetes prend le relais : le pod repart, et le run finit par passer une fois les connexions précédentes libérées. Le symptôme observable est une succession de pods `Error` puis `Completed` pour un même job, chaque run produisant tout de même une sortie correcte.
...
Au régime actuel, accélérer la pipeline passe par *plus de GPUs*, pas par un stockage plus rapide. C'est ce qui oriente l'arbitrage de coût vers la location de GPUs (cf. @couts) plutôt que vers une refonte du stockage, le NAS on-premise gardant une large marge.
...
=== Cloud ou local : synthèse par scénario
...
#figure(
  table(
    ...
    [Cluster HEIG existant], [*≈ 65*], [Électricité seule (coût marginal)],
    ...
    [Achat d'une L40S dédiée], [≈ 8'170], [8'000 achat + 107 entretien au prorata + 65 électricité],
  ),
...
#figure(
  lq.diagram(
    ...
    lq.plot((0, 5, 10, 15, 20, 25, 30), (8000, 8860, 9720, 10580, 11440, 12300, 13160), color: rgb("#9467BD"), label: [Achat d'une L40S]),
    lq.plot((0, 5, 10, 15, 20, 25, 30), (4000, 4690, 5380, 6070, 6760, 7450, 8140), color: rgb("#8C564B"), label: [Achat d'une A40]),
    lq.plot((0, 5, 10, 15, 20, 25, 30), (0, 325, 650, 975, 1300, 1625, 1950), color: rgb("#7F7F7F"), label: [Cluster HEIG existant]),
    lq.scatter((10.2, 22.7, 27.3), (9750, 11900, 12700), mark: "x", size: 9pt, color: rgb("#9467BD")),
...
#figure(
  table(
    ...
    table.header(
      ...
      text(fill: white)[*Achat L40S*#footnote[8'000 CHF d'achat, plus 800 CHF/an d'entretien à la cadence de 7,5 runs par an, plus 65 CHF d'électricité par run, soit 8'000 + 172 × n CHF.]],
    ),
    [1], [65], [350], [465], [525], [960], [4'140], [8'170],
    [5], [325], [1'750], [2'330], [2'625], [4'800], [4'690], [8'860],
    [10], [650], [3'500], [4'650], [5'250], [9'600], [5'380], [9'720],
    [15], [980], [5'250], [6'980], [7'880], [14'400], [6'070], [10'580],
    [20], [1'300], [7'000], [9'300], [10'500], [19'200], [6'760], [11'440],
    [25], [1'630], [8'750], [11'630], [13'130], [24'000], [7'450], [12'300],
    [30], [1'950], [10'500], [13'950], [15'750], [28'800], [8'140], [13'160],
  ),
...
#figure(
  lq.diagram(
    ...
    lq.plot((0, 1.5, 3, 4.5, 6, 7.5, 9), (8000, 8860, 9720, 10580, 11440, 12300, 13160), color: rgb("#9467BD"), label: [Achat d'une L40S]),
    lq.plot((0, 1.5, 3, 4.5, 6, 7.5, 9), (4000, 4690, 5380, 6070, 6760, 7450, 8140), color: rgb("#8C564B"), label: [Achat d'une A40]),
    lq.plot((0, 1.5, 3, 4.5, 6, 7.5, 9), (0, 325, 650, 975, 1300, 1625, 1950), color: rgb("#7F7F7F"), label: [Cluster HEIG existant]),
    lq.scatter((3.06, 6.81, 8.19), (9750, 11900, 12700), mark: "x", size: 9pt, color: rgb("#9467BD")),
    lq.scatter((5.67,), (6610,), mark: "x", size: 9pt, color: rgb("#8C564B")),
  ),
  caption: [Coût cumulé selon le volume total d'images annotées, à 300'000 images par run. Mêmes hypothèses et mêmes bascules que @fig-couts-runs : l'achat d'une L40S devient rentable vers 3,1 millions d'images face à OVHcloud, 6,8 face à RunPod et 8,2 face à Infomaniak, l'achat d'une A40 vers 5,7 millions face à sa propre location.],
) <fig-couts-images>

Ramené à l'image, l'écart entre les offres tient dans un ordre de grandeur. Le coût *marginal* (l'image suivante, matériel déjà payé pour les achats) distingue les options du coût *complet*, qui étale le prix d'achat sur les 9 millions d'images du scénario haut :
...
#figure(
  table(
    ...
    table.header(
      text(fill: white)[*Option*],
      text(fill: white)[*Marginal (CHF / 1'000 img)*],
      text(fill: white)[*Complet à 9 M img (CHF / 1'000 img)*],
    ),
    [Cluster HEIG existant], [0,22], [0,22],
    [Achat d'une A40], [0,46], [0,90],
    [Achat d'une L40S], [0,57], [1,46],
    [Location A40 (RunPod)], [1,17], [1,17],
    [Location L40S (Infomaniak)], [1,55], [1,55],
    [Location L40S (RunPod)], [1,75], [1,75],
    [Location L40S (OVHcloud)], [3,20], [3,20],
  ),
  caption: [Synthèse du prix par 1'000 images selon l'offre. Pour la location, marginal et complet se confondent (coût purement linéaire), pour l'achat le coût complet amortit la carte sur 9 millions d'images.],
) <tab-couts-par-image>
...
=== Le coût de l'annotation dans la chaîne complète
...
#figure(
  table(
    ...
    [Acquisition (véhicule et piétonne)], [≈ 30'000], [56 %],
    [Post-traitement (floutage, positions)], [≈ 20'000], [38 %],
    [Validation humaine assistée (≈ 63 h)], [≈ 3'150], [6 %],
    [Pré-annotation SAM3 (calcul GPU)], [≈ 22], [< 0,1 %],
  ),
  caption: [La chaîne complète pour une ville de la taille de Vevey (14'207 images), de la route à l'annotation validée. Acquisition et post-traitement sont des ordres de grandeur arrondis#footnote[Montants volontairement arrondis à partir d'un devis réel d'acquisition, dont les chiffres exacts sont confidentiels.], la validation est convertie à un taux horaire indicatif#footnote[Hypothèse de 50 CHF/h pour un annotateur, temps mesurés au @tab-score-e.].],
) <tab-chaine-complete>

L'annotation manuelle intégrale du corpus exigerait plus de 2'500 heures humaines (cf. @introduction), soit ≈ 120 heures pour une ville. L'annotation assistée réduit ce temps de 47 % (cf. @tab-score-e) : la validation retombe à ≈ 63 heures, soit ≈ 2'750 CHF économisés par ville pour ≈ 22 CHF de pré-annotation. Chaque franc de calcul GPU en économise ainsi plus de cent en temps humain, et le seul maillon de la chaîne qui se prêtait à l'automatisation est désormais son poste le moins cher.
...
=== Le démarrage intermittent du driver batch <problemes-driver>
...
=== La saturation de l'API par son propre polling
...
=== La contention des GPUs partagés
